...
function AtmQrPage() {
  ...
  const [sessionData, setSessionData] = useState(initialSession.data);
  const [timeLeft, setTimeLeft] = useState(60);
  const [status, setStatus] = useState("PENDING");
  const [error, setError] = useState("");

  // QR value: render the full session payload
  const qrValue = useMemo(() => (sessionData ? JSON.stringify(sessionData) : ""), [sessionData]);

  // Refresh QR every 60s to rotate payload
  useEffect(() => {
    if (!initialSession?.data?.sessionId) return;
    const qrInterval = setInterval(async () => {
      try {
        const res = await fetch(
          `http://localhost:8080/iam/api/qr/next?sessionId=${initialSession.data.sessionId}&includePng=false`
        );
        if (!res.ok) {
  ...
  // Poll session status: use status-with-token endpoint every 10s
  useEffect(() => {
    if (!initialSession?.data?.sessionId) return;
    ...
    const statusInterval = setInterval(async () => {
      try {
        console.log("initialSession", initialSession.data.sessionId)
        const res = await fetch(
          `http://localhost:8080/iam/api/qr/status-with-token?sessionId=${initialSession.data.sessionId}`,
        );
        if (!res.ok) return;

        const data = await res.json();
        // expected: { state: "PENDING"|"VERIFIED"|"EXPIRED", token?: string, ... }
        setStatus(data.state);

        if (data.state === "VERIFIED" && !navigating) {
          // persist any needed data, including token if provided
          const persist = {
            sessionId: initialSession.sessionId,
            ...sessionData,
            token: data.token,
          };
          localStorage.setItem("AtmsessionData", JSON.stringify(persist));
  ...
  }, [initialSession, navigate, toast, sessionData]);
